...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            images = images.cuda()
            gts = gts.cuda()
            map_list, mask_list = model(images)

            loss1 = structure_loss(map_list[0], gts)
            loss2 = structure_loss(map_list[1], gts)
            loss3 = structure_loss(map_list[2], gts)
            losse = dice_loss(mask_list[0], gts) + dice_loss(mask_list[1], gts) + dice_loss(mask_list[2], gts)
            bce_iou_deep_supervision = loss1+loss2+loss3+losse
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'CamoMS_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'CamoMS_epoch_{}.pth'.format(epoch + 1))
...
def test(test_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            gt /= (gt.max() + 1e-8)
            image = image.cuda()
            map_list, _ = model(image)
            res = map_list[0]+map_list[1]+map_list[2]
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'CamoMS.pth')
